...
```{python}
def run_efron_simulation(R=1000, n=20, seed=0):
    """
...
```{python}
efron_replication = run_efron_simulation(R=1000, n=20, seed=SEED)
...
def run_sample_size_study(sample_sizes, R=500, seed=0):
    ...
    for n in sample_sizes:
        df_n = run_efron_simulation(R=R, n=n, seed=seed+n)
        rows.append(df_n)
    return pd.concat(rows, ignore_index=True)
```

```{python}
sample_size_results = run_sample_size_study(SAMPLE_SIZES, R=500, seed=SEED)
```

For every $N$, we generate 500 independent training datasets. 
